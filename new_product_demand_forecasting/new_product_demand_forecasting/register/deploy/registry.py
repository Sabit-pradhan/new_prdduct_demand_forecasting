...
def log_and_register(model,preprocessor,run_id,cfg,train_cfg,sample_pdf):
 ...
 with tempfile.TemporaryDirectory() as d, mlflow.start_run(run_id=run_id):
  ...
  info=mlflow.pyfunc.log_model(name="model",python_model=wrapper,artifacts={"checkpoint":str(d/'checkpoint.pt'),"preprocessor":str(d/'preprocessor.json'),"model_config":str(d/'model_config.json')},input_example=example,pip_requirements=["mlflow","torch","pandas","numpy"],registered_model_name=cfg.model_fqn)
